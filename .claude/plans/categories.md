...
- [x] Добавить модель `Category` в `prisma/schema.prisma`
- [x] Добавить связь `categories` в модель `User`
- [x] Выполнить миграцию `pnpm prisma:migrate`

### 2. Entity и Repository
- [x] Создать `category.entity.ts` — типы Category
- [x] Создать `category.repository.ts` — методы работы с БД

### 3. DTO с валидацией
- [x] `create-category.dto.ts` — валидация создания
- [x] `update-category.dto.ts` — валидация обновления (IsOptional)
- [x] `category-response.dto.ts` — ответ API

### 4. CQRS Commands
- [x] `CreateCategoryCommand` + `CreateCategoryHandler`
- [x] `UpdateCategoryCommand` + `UpdateCategoryHandler`
- [x] `DeleteCategoryCommand` + `DeleteCategoryHandler`

### 5. CQRS Queries
- [x] `GetAllCategoriesQuery` + Handler — все категории пользователя
- [x] `GetCategoryByIdQuery` + Handler — одна категория (для проверки владельца)

### 6. Controller
- [x] `CategoryController` с JWT Guard
- [x] `POST /categories` — создание
- [x] `GET /categories` — список
- [x] `PATCH /categories/:id` — обновление
- [x] `DELETE /categories/:id` — удаление

### 7. Module и интеграция
- [x] Создать `CategoryModule` с импортами
- [x] Добавить `CategoryModule` в `AppModule`

### 8. Тестирование
- [x] Проверить все endpoints через curl/Postman
